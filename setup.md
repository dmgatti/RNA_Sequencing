...
This is a hands-on lesson and students must have R and RStudio installed on 
their computers. They also need to be able to install a number of R packages, 
create directories, and download files.

To avoid troubleshooting during the lesson, students should follow the
...
- When you open RStudio your R version will be printed in the console on
  ...
  If your R version is 4.3.1 or later, you don't need to update R for this
...
Then load the [BiocManager]() package and use it to install bioinformatics
packages.

```r
library(BiocManager)
BiocManager::install(c('AnnotationHub', 'DESeq2', 'GenomicRanges'))
```
...
The data files for the lesson can be downloaded manually here: TBD